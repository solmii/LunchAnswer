...
const ResultForm = ({ history }) => {
	...
	return (
		<ResultSection>
			<ResultNav>
				<img alt='go_home' src={sub_logo} onClick={() => history.push('/intro')} />
			</ResultNav>
					...
					{filteredFoodList.map((food) => (
						<>
							<li className='alignLeft'>
								<OtherChat>
									그럼 오늘 점심으로 <span className='pointText'>{food.food_name}</span> 어때?
								</OtherChat>
							</li>
							...
							<li className='alignLeft'>
								<OtherChat isDelay={'2.3s'}>
									네가 말해준 조건과 <span className='pointText'>75%</span> 일치해!
								</OtherChat>
							</li>
						</>
					))}
					...
					<li>
						...
						>
							<div>
								오늘 점심은
								<br />
								<b>{foodResult.length > 1 && filteredFoodList[resultView].food_name}</b>!!
							</div>
						</Button>
					</li>
					<li>
						<Button color='gray' mouseAction={mouseAction} eventFunc={() => console.log('NO')}>
							<div>
								<b>다른 메뉴</b>는
								<br />
								없어?
